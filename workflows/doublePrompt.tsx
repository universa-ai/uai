...
export default async function (agentAndTask, targetPath) {
  const [agentCreationPrompt, taskPrompt] = agentAndTask;

  const agentPrompt = await prompt(
    <>
      <settings temperature={0.5} model="gpt-4o" />
      <system>
        You are prompt engineer, given a vague description of a new agent, you have to write a complete detailed system
  ...
  if (generatedTasks.writtenByArchitectDeveloper) {
    agents.push(
