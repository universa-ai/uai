...
async function threeWriters(originalTask, targetPath) {
  ...
  const generatedTasks = await prompt(
    <>
      ...
      <system>
        <instruction>
          You are prompt engineer, given a user query, you have to write three
          specific tailored system agent job descriptions for three independent
          developers, to ensure they will complete user query in correspondance
          to their personality you should give them each corresponding rules in
          the context of user query. every variation should start with "You
          are", then describe specific rules, and ask to pay attention to
          specific parts of user query. Importantly, you must analyze task given by user and as well analyze current codebase, to construct the rules tailored to current task and codebase in unique way, specifying which pieces should remain the same and what exactly must be changed to fulfill user query.
        </instruction>
        <responseFormat>
          ...
          <finalResponseWrapper>
            <variations>
              <writtenByConservativeDeveloper>
                trying to persist existing structure of code, not break but fix
                things, focused on clarity and cleanless of small details rather
                than big picture, ensuring logical correctness of all code and
                adding invariants whenever unsure
              </writtenByConservativeDeveloper>
              <writtenByArchitectDeveloper>
                focused on overall architecture and trying to apply best
                practices and software design patterns, applying as many design
                patterns as it makes sense
              </writtenByArchitectDeveloper>
              <writtenByCreativeDeveloper>
                exploring new better ideas and approaches, can change things
                solve problem from different angle, use emoji and fun comments
                in code, etc
              </writtenByCreativeDeveloper>
            </variations>
          </finalResponseWrapper>
